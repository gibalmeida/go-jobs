...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withDepartments; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.Departments = []*Department{}
		}
		query.withFKs = true
		query.Where(predicate.Department(func(s *sql.Selector) {
			s.Where(sql.InValues(user.DepartmentsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_departments" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_departments" returned %v for node %v`, *fk, n.ID)
			}

...
func (jq *JobQuery) QueryDepartment() *DepartmentQuery {
	query := &DepartmentQuery{config: jq.config}
...
func (jq *JobQuery) Clone() *JobQuery {
	...
	return &JobQuery{
		...
		withDepartment: jq.withDepartment.Clone(),
...
func (jq *JobQuery) sqlAll(ctx context.Context) ([]*Job, error) {
	var (
		...
		_spec       = jq.querySpec()
		loadedTypes = [1]bool{
			jq.withDepartment != nil,
		}
	)
	if jq.withDepartment != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := jq.withDepartment; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Job)
		for i := range nodes {
			if fk := nodes[i].department_jobs; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "department_jobs" returned %v`, n.ID)
			}
			for i := range nodes {

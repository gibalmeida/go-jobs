...
func (dq *DepartmentQuery) QueryJobs() *JobQuery {
	query := &JobQuery{config: dq.config}
...
func (dq *DepartmentQuery) sqlAll(ctx context.Context) ([]*Department, error) {
	...
	if query := dq.withJobs; query != nil {
		...
		query.Where(predicate.Job(func(s *sql.Selector) {
			s.Where(sql.InValues(department.JobsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.department_jobs
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "department_jobs" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "department_jobs" returned %v for node %v`, *fk, n.ID)
			}
	...
	if query := dq.withManager; query != nil {
		...
		nodeids := make(map[int][]*Department)
		for i := range nodes {
			if fk := nodes[i].user_departments; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_departments" returned %v`, n.ID)
			}
			for i := range nodes {

...
func (d *Department) Node(ctx context.Context) (node *Node, err error) {
	...
	node.Edges[0].IDs, err = d.QueryJobs().
		Select(job.FieldID).
		Ints(ctx)
	if err != nil {
		return nil, err
	...
	node.Edges[1].IDs, err = d.QueryManager().
		Select(user.FieldID).
		Ints(ctx)
	if err != nil {
		return nil, err
...
func (j *Job) Node(ctx context.Context) (node *Node, err error) {
	...
	if buf, err = json.Marshal(j.Description); err != nil {
		return nil, err
	}
	node.Fields[1] = &Field{
		Type:  "string",
	...
	node.Edges[0].IDs, err = j.QueryDepartment().
...
func (u *User) Node(ctx context.Context) (node *Node, err error) {
	...
	node.Edges[0].IDs, err = u.QueryDepartments().
		Select(department.FieldID).
		Ints(ctx)
	if err != nil {
		return nil, err
...
func (c *Client) noder(ctx context.Context, table string, id int) (Noder, error) {
	...
	case department.Table:
		n, err := c.Department.Query().
			Where(department.ID(id)).
			CollectFields(ctx, "Department").
			Only(ctx)
		if err != nil {
			return nil, err
		}
